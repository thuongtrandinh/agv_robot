...
def generate_launch_description():
    ...
    use_robot_state_pub = LaunchConfiguration('use_robot_state_publisher', default='true')
    ...
    # motor_odom publishes /diff_cont/odom and broadcasts odom->base_footprint
    motor_odom = Node(
    ...
    # arguments (kept for compatibility)
    ld.add_action(DeclareLaunchArgument('use_robot_state_publisher', default_value='true'))

    # Add nodes. For a minimal robot init we include state publisher, controller manager, spawner, motor_odom, and static tf
    ld.add_action(robot_state_pub)
    ld.add_action(controller_manager)
    ld.add_action(spawner_jsb)
    ld.add_action(motor_odom)
