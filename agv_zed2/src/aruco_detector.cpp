...
public:
          ...
          dict_(cv::aruco::getPredefinedDictionary(cv::aruco::DICT_6X6_250))
    {
        ...
        // Open camera (ZED2 on IPC: /dev/video0 outputs side-by-side stereo 1344×376)
        cap_left_.open(device_, cv::CAP_V4L2);
        if (!cap_left_.isOpened()) {
            RCLCPP_FATAL(get_logger(), "❌ Cannot open camera %s", device_.c_str());
            RCLCPP_FATAL(get_logger(), "💡 Tip: Check if device is busy with: sudo fuser -v %s", device_.c_str());
            rclcpp::shutdown();
            return;
        }

        // Set format for stereo capture (side-by-side)
        cap_left_.set(cv::CAP_PROP_FOURCC, cv::VideoWriter::fourcc('Y','U','Y','V'));
        cap_left_.set(cv::CAP_PROP_FRAME_WIDTH, stereo_width_);   // 1344 for side-by-side
        ...
        if (actual_w == stereo_width_ && use_stereo_) {
            RCLCPP_INFO(get_logger(), "✅ Side-by-side stereo mode detected");
        } else if (actual_w == img_width_) {
            RCLCPP_WARN(get_logger(), "⚠️  Got mono resolution, stereo disabled");
            use_stereo_ = false;
        } else {
            RCLCPP_WARN(get_logger(), "⚠️  Unexpected resolution %dx%d, using mono mode", actual_w, actual_h);
            use_stereo_ = false;
        }
...
private:
    ...
    cv::VideoCapture cap_left_;   // ZED2 camera (side-by-side stereo on /dev/video0)
